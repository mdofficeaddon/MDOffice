...
- **🖼️ Image** - Insert image ✨ **Enhanced!**
  - Browse local files (PNG, JPG, GIF, SVG, etc.)
  - Enter image URL
  - Drag-and-drop from file explorer ✅ **NEW**
  - Paste from clipboard (Ctrl+V) ✅ **NEW**
  - Auto-save to images/ folder
  - Support for relative paths, absolute paths, and base64
- **📊 Table** - Insert table
...
### Enhanced Image Support (NEW! v0.2.6)

**MDOffice now includes comprehensive image insertion capabilities!**

#### 4 Ways to Insert Images:

1. **🖼️ Image Button**: Click the toolbar button for options:
   - **Browse Files**: Select from your file system (PNG, JPG, GIF, SVG, BMP, WEBP)
   - **Enter URL**: Use online images from any URL
   - **Clipboard**: Instructions for paste functionality

2. **🎯 Drag & Drop**: 
   - Drag any image file from your file explorer
   - Drop it directly into the editor or preview
   - Image automatically saved to `images/` folder
   - Markdown link inserted with relative path

3. **📋 Clipboard Paste**:
   - Copy any image (from browser, screenshot tool, etc.)
   - Press `Ctrl+V` (or `Cmd+V` on Mac) in MDOffice
   - Image saved as `images/pasted-image-[timestamp].png`
   - Perfect for quick screenshots!

4. **💾 Multiple Path Options**:
   - **Relative Path** (recommended): `./images/photo.jpg` - works everywhere
   - **Absolute Path**: Full system path - works locally only
   - **Base64 Embed**: Encoded directly in markdown - increases file size

#### Image Features:
- ✅ Automatic path resolution for local images
- ✅ Support for HTTP, HTTPS, data URIs, and file:// protocols
- ✅ Images work in lists, tables, blockquotes, and inline
...
- [x] **Image drag-and-drop from file system** ✅ **NEW v0.2.6**
- [x] **Image paste from clipboard** ✅ **NEW v0.2.6**
- [x] **Enhanced image insertion with file browser and base64** ✅ **NEW v0.2.6**
